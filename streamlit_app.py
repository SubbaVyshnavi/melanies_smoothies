...
my_data=session.table("SMOOTHIES.PUBLIC.FRUIT_OPTIONS").select(col("fruit_name"),col("search_on"))
st_pd=my_data.to_pandas()
...
if ing:
    ...
    for food in ing:
        in_str += food + ' '
        search_on=pd_df.loc[pd_df['FRUIT_NAME'] == food, 'SEARCH_ON'].iloc[0]
